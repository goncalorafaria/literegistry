...
RECOMMENDED USAGE (most efficient):
    uvicorn literegistry.gateway.legacy:create_app --factory --host 0.0.0.0 --port 8080 --workers 8

ALTERNATIVE USAGE (for development/testing):
    python -m literegistry.gateway.legacy
...
class StarletteGatewayServer:
    ...
    def start_with_workers(self, workers: int = 1):
        ...
        uvicorn.run(
            "literegistry.gateway.legacy:create_app",
            host=self.host,
            port=self.port,
            workers=workers,
            log_level="info",
