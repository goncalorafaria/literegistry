...
DEFAULT_JINA_READER_URL = "https://r.jina.ai/"
...
class SearchServerConfig:
    ...
    # ``mode=url`` is deliberately served by Jina Reader, not Serper's scrape
    # endpoint. Query and URL requests therefore share one registered
    # ``search`` worker while retaining their provider-specific credentials.
    fetch_api_url: str | None = DEFAULT_JINA_READER_URL
...
class SearchServer:
    ...
    def _cache_key(self, mode: str, endpoint: str, payload: dict[str, Any]) -> str:
        canonical = json.dumps(
            {
                "mode": mode,
                "endpoint": endpoint,
                # Do not include either provider's credential in a cache key.
                # URL request parameters are represented in ``payload`` below.
                "headers": (
                    self.config.upstream_headers
                    if mode == "query"
                    else {"X-Return-Format": self.config.jina_return_format}
                ),
                "payload": payload,
            },
            sort_keys=True,
    ...
    def _headers_for_request(self, request: SearchRequest) -> dict[str, str]:
        if request.mode == "query":
            return self.config.upstream_headers
        ...
        headers = {
            ...
            "X-Return-Format": self.config.jina_return_format,
        }
    ...
    @staticmethod
    def _normalize_jina_data(data: Any, url: str) -> dict[str, str]:
        """Return the stable FetchClient shape from a Jina Reader response."""
        ...
        if not isinstance(data, dict):
            raise HTTPException(status_code=502, detail="Jina Reader returned an invalid body")
        content = data.get("content") or data.get("markdown") or data.get("text") or ""
        title = data.get("title") or ""
        return {"title": str(title), "content": str(content), "url": url}
    ...
    async def execute(self, request: SearchRequest) -> SearchResponse:
        ...
        if request.mode == "url":
            # Jina accepts request parameters as headers, but those headers can
            # influence the result and must participate in cache identity.
            cache_payload = {**payload, "parameters": request.parameters}
        cache_key = self._cache_key(request.mode, endpoint, cache_payload)
        ...
        try:
            async with self.session.post(
                ...
                if request.mode == "url":
                    ...
                    data = self._normalize_jina_data(data, request.url)
...
def main(
    ...
    """Run one cached Serper/generic search worker with Jina URL visits."""
    import uvicorn
    ...
    if provider == "serper":
        ...
        fetch_api_url = fetch_api_url or os.getenv("JINA_READER_URL") or DEFAULT_JINA_READER_URL
    else:
        search_api_url = search_api_url or os.getenv("SEARCH_API_URL")
        fetch_api_url = fetch_api_url or os.getenv("JINA_READER_URL") or DEFAULT_JINA_READER_URL
    if not search_api_url or not fetch_api_url:
        raise ValueError(
            "Both search_api_url and the Jina Reader URL are required "
            "(or set SEARCH_API_URL and JINA_READER_URL)"
        )
    ...
    if not jina_api_key:
        raise ValueError("JINA_API_KEY is required for URL fetches")
